...
def init_strings():
    ...
    G.empty_unibyte_string = make_pure_string (b"", 0, 0, false)
    #   staticpro (&empty_unibyte_string);
    #   empty_multibyte_string = make_pure_string ("", 0, 0, 1);
    G.empty_multibyte_string = make_pure_string (b"", 0, 0, true)
...
# {
def allocate_string() -> Lisp_String:
    # struct Lisp_String *s;
    #
    # MALLOC_BLOCK_INPUT;
    #
...
# {
def allocate_string_data(s: Lisp_String, nchars: int, nbytes: int, clearit: bool):
    # sdata *data;
    # struct sblock *b;
    #
...
# {
def make_string(contents: Union[str, bytes], nbytes: int) -> Lisp_String:
    # register Lisp_Object val;
    # ptrdiff_t nchars, multibyte_nbytes;
    #
    # parse_str_as_multibyte ((const unsigned char *) contents, nbytes,
    #                         &nchars, &multibyte_nbytes);
    contents, nchars, multibyte_nbytes = parse_str_as_multibyte(contents, nbytes)
    # if (nbytes == nchars || nbytes != multibyte_nbytes)
    if nbytes == nchars or nbytes != multibyte_nbytes:
...
# {
def make_unibyte_string(contents: bytes, length: int):
    # register Lisp_Object val;
    # val = make_uninit_string (length);
    val = make_uninit_string(length)
    # memcpy (SDATA (val), contents, length);
    val.u.s.data[:] = contents
    # return val;
    return val
# }
#
...
#   return val;
# }
def make_multibyte_string(contents: bytes, nchars: int, nbytes: int):
    val = make_uninit_multibyte_string(nchars, nbytes)
    val.u.s.data[:] = contents
    return val
#
...
#   return val;
# }
def make_clear_string(length: int, clearit: bool):
    if not length:
        return G.empty_unibyte_string
    val = make_clear_multibyte_string(length, length, clearit)
    val = STRING_SET_UNIBYTE(val)
    return val
#
# /* Return a unibyte Lisp_String set up to hold LENGTH characters
...
# }
def make_uninit_string(length: int):
    return make_clear_string(length, false)
#
#
...
# {
def make_clear_multibyte_string(nchars: int, nbytes: int, clearit: bool):
    # Lisp_Object string;
    # struct Lisp_String *s;
    #
    # if (nchars < 0)
    ...
    if not nbytes:
        return G.empty_multibyte_string
    #
    # s = allocate_string ();
    s = allocate_string()
...
# }
def make_uninit_multibyte_string(nchars: int, nbytes: int):
    return make_clear_multibyte_string(nchars, nbytes, false)
#
...
class F:
    ...
    # {
    @staticmethod
    def cons(car: Lisp_Object, cdr: Lisp_Object):
        #   register Lisp_Object val;
        #
        #   MALLOC_BLOCK_INPUT;
        #
...
def list5(arg1, arg2, arg3, arg4, arg5):
    return F.cons(arg1, F.cons(arg2, F.cons(arg3, F.cons(arg4, F.cons(arg5, Q.nil)))))
#
...
#   return val;
# }
def listn(*args: Lisp_Object) -> Lisp_Cons:
    """Make a list of Lisp_Objects."""
    val = cons_listn(args, F.cons)
    return val
#
...
#   return val;
# }
def pure_listn(*args: Lisp_Object) -> Lisp_Cons:
    """Make a pure list of Lisp_Objects."""
    val = cons_listn(args, pure_cons)
    return val
...
class F:
    ...
    # {
    @classmethod
    def make_symbol(cls, name: Lisp_Object):
        # Lisp_Object val;
        #
        # CHECK_STRING (name);
        CHECK_STRING(name)
...
# {
def pure_alloc(kind: Type[TLisp]) -> TLisp:
    return kind()
#   void *result;
#
#  again:
...
# {
def find_string_data_in_pure(data: bytes, nbytes: int):
    return bytearray(data[:nbytes])
#   int i;
...
#   return string;
# }
def make_pure_string(data: bytes, nchars: int, nbytes: int, multibyte: bool) -> Lisp_String:
    s = pure_alloc(Lisp_String)
    s.u.s.data = find_string_data_in_pure(data, nbytes)
    if s.u.s.data == NULL:
        # s.u.s.data = pure_alloc(nbytes + 1, -1)
        s.u.s.data = bytearray(nbytes + 1)
        s.u.s.data[:] = -1
        memcpy(s.u.s.data, 0, bytearray(data), 0, nbytes)
        s.u.s.data[nbytes] = 0
    s.u.s.size = nchars
    s.u.s.size_byte = nbytes if multibyte else -1
    s.u.s.intervals = NULL
    string = XSETSTRING(s)
    return string
#
...
#   return string;
# }
def make_pure_c_string(data: str, nchars: int):
    s = pure_alloc(Lisp_String)
    s.u.s.size = nchars
    s.u.s.size_byte = -2
    s.u.s.data = bytearray(data.encode('utf-8') + b'\0')
    s.u.s.intervals = NULL
    string = XSETSTRING(s)
    return string
#
...
#   return new;
# }
def pure_cons(car: Lisp_Object, cdr: Lisp_Object) -> Lisp_Cons:
    """Return a cons allocated from pure space.  Give it pure copies
    of CAR as car and CDR as cdr."""
    new = pure_alloc(Lisp_Cons)
    XSETCAR(new, purecopy(car))
    XSETCDR(new, purecopy(cdr))
    return new
