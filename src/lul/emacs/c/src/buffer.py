...
class G_:
    ...
    @classproperty
    def BEG_ADDR(self):
        return G.current_buffer.text.beg
    BEG_ADDR: ClassVar[bytearray]
    #
    # /* Address of beginning of accessible range of buffer.  */
    # #define BEGV_ADDR (BYTE_POS_ADDR (current_buffer->begv_byte))
    @classproperty
    def BEGV_ADDR(self):
        return BYTE_POS_ADDR(G.current_buffer.begv_byte)
    BEGV_ADDR: ClassVar[bytearray]
    #
    ...
    @staticmethod
    def BUF_Z(buf: buffer):
        return buf.text.z
    @staticmethod
    def BUF_Z_SET(buf: buffer, z: int):
        buf.text.z = z
    # #define BUF_Z_BYTE(buf) ((buf)->text->z_byte)
    @staticmethod
    def BUF_Z_BYTE(buf: buffer):
        return buf.text.z_byte
    @staticmethod
    def BUF_Z_BYTE_SET(buf: buffer, z_byte: int):
        buf.text.z_byte = z_byte
    #
    # /* Address of beginning of buffer.  */
    # #define BUF_BEG_ADDR(buf) ((buf)->text->beg)
    @staticmethod
    def BUF_BEG_ADDR(buf: buffer):
        return buf.text.beg
    #
    # /* Size of gap.  */
...
class buffer:
    ...
    enable_multibyte_characters_: Lisp_Object = Q.nil
    #
    ...
    own_text: buffer_text = dataclasses.field(default_factory=buffer_text)
    #
...
# }
def BUFFERP(a: Lisp_Object):
    return isinstance(a, buffer)
...
# }
def CHECK_BUFFER(x: Lisp_Object):
    return CHECK_TYPE(BUFFERP(x), Q.bufferp, x)
...
# }
def XBUFFER(a: Lisp_Object) -> buffer:
    assert BUFFERP(a)
    return a
#
...
# }
def bset_enable_multibyte_characters(b: buffer, val: Lisp_Object):
    b.enable_multibyte_characters_ = val
# INLINE void
...
# }
def BYTE_POS_ADDR(n: int) -> bytearray:
    return G.BEG_ADDR[n - BEG_BYTE:]
#
...
# }
def CHAR_TO_BYTE(charpos):
    """Convert a character position to a byte position."""
    return buf_charpos_to_bytepos(G.current_buffer, charpos)
#
...
# }
def BUFFER_LIVE_P(b: buffer):
    return not NILP(BVAR(b, "name"))
#
...
# }
def set_buffer_internal(b: buffer):
...
#   b->text->intervals = i;
# }
def set_buffer_intervals(b: buffer, i: INTERVAL):
    assert b.text != NULL
    b.text.intervals = i
#
