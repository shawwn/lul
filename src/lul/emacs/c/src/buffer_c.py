...
# {
def fix_position(pos: Lisp_Object) -> int:
    # if (FIXNUMP (pos))
    #   return XFIXNUM (pos);
    if FIXNUMP(pos):
        return XFIXNUM(pos)
    ...
    CHECK_TYPE(BIGNUMP(pos), Q.integer_or_marker_p, pos)
    # return !NILP (Fnatnump (pos)) ? MOST_POSITIVE_FIXNUM : MOST_NEGATIVE_FIXNUM;
    return MOST_POSITIVE_FIXNUM if not NILP(F.natnump(pos)) else MOST_NEGATIVE_FIXNUM
# }
#
...
#   b->name_ = val;
# }
def bset_name(b: buffer, val: Lisp_Object):
    b.name_ = val
# static void
...
# {
def assoc_ignore_text_properties(key: Lisp_Object, list: Lisp_Object):
    # Lisp_Object tail;
    ...
    for tail, set_tail in FOR_EACH_TAIL(list):
        ...
        #   return elt;
        if not NILP(F.string_equal(F.car(elt), key)):
            return elt
    #   }
    # return Qnil;
    return Q.nil
...
class F:
    ...
    # {
    @DEFUN("get-buffer", S.get_buffer, 1, 1, 0)
    @staticmethod
    def get_buffer(buffer_or_name: Lisp_Object):
        ...
        return F.cdr(assoc_ignore_text_properties(buffer_or_name, V.buffer_alist))
    # }
#
...
class F:
    ...
    # {
    @DEFUN("get-buffer-create", S.get_buffer_create, 1, 2, 0)
    @staticmethod
    def get_buffer_create(buffer_or_name: Lisp_Object, inhibit_buffer_hooks: Lisp_Object):
        ...
        V.buffer_alist = nconc2(V.buffer_alist, list1(F.cons(name, buffer)))
        #
...
def reset_buffer(b: buffer):
    ...
    bset_enable_multibyte_characters(b, Q.nil) # TODO: BVAR (&buffer_defaults, enable_multibyte_characters)
...
# {
def reset_buffer_local_variables(b: buffer, permanent_too: bool):
    # int offset, i;
    #
...
class F:
    ...
    #   (void)
    # {
    @DEFUN("current-buffer", S.current_buffer, 0, 0, 0)
    @staticmethod
    def current_buffer():
...
# }
def set_buffer_internal_1(b: buffer):
    if G.current_buffer == b:
        return
    set_buffer_internal_2(b)
#
...
# {
def set_buffer_internal_2(b: buffer):
...
class F:
    ...
    #   (register Lisp_Object buffer_or_name)
    # {
    @DEFUN("set-buffer", S.set_buffer, 1, 1, 0)
    @staticmethod
    def set_buffer(buffer_or_name: Lisp_Object):
        # register Lisp_Object buffer;
        # buffer = Fget_buffer (buffer_or_name);
        buffer = F.get_buffer(buffer_or_name)
        # if (NILP (buffer))
        ...
        if not BUFFER_LIVE_P(XBUFFER(buffer)):
            error ("Selecting deleted buffer")
...
# {
def validate_region(b: Lisp_Object, e: Lisp_Object):
    ...
    if not (G.BEGV <= beg and end <= G.ZV):
        args_out_of_range_3(F.current_buffer(), b, e)
    #
    # *b = make_fixnum (beg);
    # *e = make_fixnum (end);
    b = make_fixnum(beg)
...
def init_buffer_once():
    ...
    #
    # Vbuffer_alist = Qnil;
    V.buffer_alist = Q.nil
    # current_buffer = 0;
    G.current_buffer = NULL
    ...
    V.prin1_to_string_buffer = F.get_buffer_create(build_pure_c_string(" prin1"), Q.t)
    # Vbuffer_alist = Qnil;
    V.buffer_alist = Q.nil
    #
    # Fset_buffer (Fget_buffer_create (build_pure_c_string ("*scratch*"), Qnil));
    F.set_buffer(F.get_buffer_create(build_pure_c_string("*scratch*"), Q.nil))
    #
