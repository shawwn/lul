...
# {
def make_buffer_string(start: int, end: int, props: bool):
...
# {
def make_buffer_string_both(start: int, start_byte: int, end: int, end_byte: int, props: bool):
    ...
    if not NILP(BVAR(G.current_buffer, "enable_multibyte_characters")):
        result = make_uninit_multibyte_string(end - start, end_byte - start_byte)
    else:
        result = make_uninit_string(end - start)
    ...
    memcpy (SDATA (result), 0, BYTE_POS_ADDR (beg0), 0, size)
    # if (beg1 != -1)
    #   memcpy (SDATA (result) + size, BEG_ADDR + beg1, end1 - beg1);
    if beg1 != -1:
        memcpy (SDATA (result), size, G.BEG_ADDR, beg1, end1 - beg1)
    #
    ...
    #   }
    if props:
        print(NotImplementedError("make_buffer_string_both(props)"))
    #
    return result
# }
#
...
class F:
    ...
    # {
    @DEFUN("buffer-substring", S.buffer_substring, 2, 2, 0)
    @staticmethod
    def buffer_substring(start: Lisp_Object, end: Lisp_Object):
        # register ptrdiff_t b, e;
        #
        # validate_region (&start, &end);
        start, end = validate_region(start, end)
        # b = XFIXNUM (start);
        b = XFIXNUM(start)
        # e = XFIXNUM (end);
        e = XFIXNUM(end)
    ...
    #   (void)
    # {
    @DEFUN("buffer-string", S.buffer_string, 0, 0, 0)
    @staticmethod
    def buffer_string():
        #   return make_buffer_string_both (BEGV, BEGV_BYTE, ZV, ZV_BYTE, 1);
        return make_buffer_string_both(G.BEGV, G.BEGV_BYTE, G.ZV, G.ZV_BYTE, True)
    # }
    #
