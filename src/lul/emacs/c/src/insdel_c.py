...
# {
def move_gap_both(charpos: ptrdiff_t, bytepos: ptrdiff_t):
    print("TODO: move_gap_both")
...
# {
def adjust_markers_for_insert(from_: ptrdiff_t, from_byte: ptrdiff_t,
                              to: ptrdiff_t, to_byte: ptrdiff_t, before_markers: bool_t):
    print("TODO: adjust_markers_for_insert")
#   struct Lisp_Marker *m;
#   bool adjusted = 0;
...
# }
def adjust_point(nchars: ptrdiff_t, nbytes: ptrdiff_t):
    SET_BUF_PT_BOTH(G.current_buffer, G.PT + nchars, G.PT_BYTE + nbytes)
    assert G.PT_BYTE >= G.PT and G.PT_BYTE - G.PT <= G.ZV_BYTE - G.ZV
# 
...
# {
def make_gap(nbytes_added: ptrdiff_t):
    print("TODO: make_gap")
#   if (nbytes_added >= 0)
...
# {
def insert_1_both(string: bytes, nchars: int, nbytes: int,
                  inherit: bool_t, prepare: bool_t, before_markers: bool_t):
    # if (nchars == 0)
    #   return;
    if nchars == 0:
        return
    ...
    # if (NILP (BVAR (current_buffer, enable_multibyte_characters)))
    #   nchars = nbytes;
    if NILP(BVAR(G.current_buffer, "enable_multibyte_characters")):
        nchars = nbytes
    #
    # if (prepare)
    if prepare:
        ...
        prepare_to_modify_buffer(G.PT, G.PT)
    #
    # if (PT != GPT)
    #   move_gap_both (PT, PT_BYTE);
    if G.PT != G.GPT:
        move_gap_both(G.PT, G.PT_BYTE)
    # if (GAP_SIZE < nbytes)
    #   make_gap (nbytes - GAP_SIZE);
    if G.GAP_SIZE < nbytes:
        make_gap(nbytes - G.GAP_SIZE)
    #
    #ifdef BYTE_COMBINING_DEBUG
    ...
    record_insert (G.PT, nchars)
    # modiff_incr (&MODIFF);
    G.MODIFF = modiff_incr(G.MODIFF)
    # CHARS_MODIFF = MODIFF;
    G.CHARS_MODIFF = G.MODIFF
    #
    # memcpy (GPT_ADDR, string, nbytes);
    memcpy(G.GPT_ADDR, 0, string, 0, nbytes)
    #
    # GAP_SIZE -= nbytes;
    G.GAP_SIZE -= nbytes
    # GPT += nchars;
    ...
    # if (GAP_SIZE > 0) *(GPT_ADDR) = 0; /* Put an anchor.  */
    if G.GAP_SIZE > 0:
        G.GPT_ADDR[0] = 0 # Put an anchor
    #
    # eassert (GPT <= GPT_BYTE);
    assert G.GPT <= G.GPT_BYTE
    #
    # /* The insert may have been in the unchanged region, so check again.  */
    # if (Z - GPT < END_UNCHANGED)
    #   END_UNCHANGED = Z - GPT;
    if G.Z - G.GPT < G.END_UNCHANGED:
        G.END_UNCHANGED = G.Z - G.GPT
    #
    # adjust_overlays_for_insert (PT, nchars);
    adjust_overlays_for_insert(G.PT, nchars)
    # adjust_markers_for_insert (PT, PT_BYTE,
    #                            PT + nchars, PT_BYTE + nbytes,
    #                            before_markers);
    adjust_markers_for_insert(G.PT, G.PT_BYTE,
                              G.PT + nchars, G.PT_BYTE + nbytes,
                              before_markers)
    #
    # offset_intervals (current_buffer, PT, nchars);
    offset_intervals(G.current_buffer, G.PT, nchars)
    #
    # if (!inherit && buffer_intervals (current_buffer))
    #   set_text_properties (make_fixnum (PT), make_fixnum (PT + nchars),
    #                        Qnil, Qnil, Qnil);
    if not inherit and buffer_intervals(G.current_buffer):
        set_text_properties(make_fixnum(G.PT), make_fixnum(G.PT + nchars),
                            Q.nil, Q.nil, Q.nil)
    #
...
# }
def del_range(from_: int, to: int):
    print("TODO: del_range")
#
...
#                           ptrdiff_t *preserve_ptr)
# {
def prepare_to_modify_buffer(start: ptrdiff_t, end: ptrdiff_t):
    print("TODO: prepare_to_modify_buffer")
...
# {
def signal_after_change(charpos: ptrdiff_t, lendel: ptrdiff_t, lenins: ptrdiff_t):
    print("TODO: signal_after_change")
    #   ptrdiff_t count = SPECPDL_INDEX ();
    #   struct rvoe_arg rvoe_arg;
