...
#   return obarray;
# }
def check_obarray(obarray: Lisp_Object) -> Lisp_Obarray:
    assert isinstance(obarray, list) and (len(obarray) == 0 or obarray[-1] == 0 or isinstance(obarray[-1], Lisp_Symbol))
    return obarray
...
def intern_sym(sym: Lisp_Object, obarray: Lisp_Object):
    ...
    #                                  : SYMBOL_INTERNED);
    XSYMBOL(sym).u.s.interned = (SYMBOL_INTERNED_IN_INITIAL_OBARRAY
                                 if EQ(obarray, G.initial_obarray) else
                                 SYMBOL_INTERNED)
    #
    #   if (SREF (SYMBOL_NAME (sym), 0) == ':' && EQ (obarray, initial_obarray))
    #     {
    if SREF(SYMBOL_NAME(sym), 0) == b':'[0] and EQ(obarray, G.initial_obarray):
...
# }
def intern_driver(string: Lisp_Object, obarray: Lisp_Object):
    """Intern a symbol with name STRING in OBARRAY."""
    return intern_sym(F.make_symbol(string), obarray)
...
def define_symbol(sym: Lisp_Object, name: str):
    ...
    if not EQ(sym, Q.unbound):
        ...
        intern_sym(sym, G.initial_obarray)
    #     }
...
class F:
    ...
    # {
    @classmethod
    def intern(cls, string: Lisp_Object, obarray: Lisp_Object = Q.nil):
        # Lisp_Object tem;
        #
        # obarray = check_obarray (NILP (obarray) ? Vobarray : obarray);
        obarray = check_obarray(V.obarray if NILP(obarray) else obarray)
        # CHECK_STRING (string);
        CHECK_STRING(string)
        #
        #
        ...
        if not SYMBOLP(tem):
            ...
            #                         obarray, tem);
            tem = intern_driver(string if NILP(V.purify_flag) else F.purecopy(string), obarray)
        #   }
        # return tem;
        return tem
    # }
...
# {
def oblookup(obarray: Lisp_Object, ptr: str) -> Optional[Lisp_Symbol]:
    # size_t hash;
    # size_t obsize;
...
# {
def oblookup_considering_shorthand(obarray: Lisp_Obarray, in_: str) -> Optional[Lisp_Symbol]:
    return oblookup(obarray, in_)
...
def init_obarray_once():
    ...
    V.obarray = make_vector(PP.OBARRAY_SIZE, make_fixnum (0))
    ...
    SET_SYMBOL_VAL(XSYMBOL(Q.unbound), CV.Token.MISSING)
