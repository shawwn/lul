...
if c_ifndef("ENABLE_CHECKING"):
    @PP_
    class PP(PP):
        # # define eassert(cond) ((void) (false && (cond))) /* Check COND compiles.  */
        @staticmethod
        def eassert(cond: str, globals: dict, locals: dict):
            compile(cond, "eassert", "eval") # Check COND compiles.

        # # define eassume(cond) assume (cond)
        @staticmethod
        def eassume(cond: str, globals: dict, locals: dict):
            return assume(cond, globals, locals)
    ...
    class G(G):
        @staticmethod
        def die(msg: str, file: str = None, line: int = None):
            breakpoint()

    # extern bool suppress_checking EXTERNALLY_VISIBLE;
    @G_
    class G(G):
        suppress_checking: bool = False
    ...
    class PP(PP):
        @staticmethod
        def eassert(cond: str, globals: dict, locals: dict):
            if G.suppress_checking or eval(cond, globals, locals):
                pass
            else:
                return G.die(cond, globals.get('__file__', None))
    ...
    class PP(PP):
        @staticmethod
        def eassume(cond: str, globals: dict, locals: dict):
            if G.suppress_checking:
                return assume(cond, globals, locals)
            elif eval(cond, globals, locals):
                pass
            else:
                return G.die(cond, globals.get('__file__', None))
...
# }
def CHECK_TYPE(ok, predicate: Lisp_Object, x: Lisp_Object):
    if not ok:
...
class symbol_interned(_enum.IntEnum):
    SYMBOL_UNINTERNED = 0
    SYMBOL_INTERNED = 1
    SYMBOL_INTERNED_IN_INITIAL_OBARRAY = 2
...
class symbol_redirect(_enum.IntEnum):
    SYMBOL_PLAINVAL = 4
    SYMBOL_VARALIAS = 1
    SYMBOL_LOCALIZED = 2
    SYMBOL_FORWARDED = 3
...
class symbol_trapped_write(_enum.IntEnum):
    SYMBOL_UNTRAPPED_WRITE = 0
    SYMBOL_NOWRITE = 1
    SYMBOL_TRAPPED_WRITE = 2
...
class Lisp_Symbol(Generic[TLisp]):
    ...
    class _val(Generic[TLisp]):
        value: Lisp_Object = None
        alias: Lisp_Symbol = None
...
# }
def SYMBOLP(x: Lisp_Object):
    return isinstance(x, Lisp_Symbol)
...
#   return p;
# }
def XSYMBOL(a: Lisp_Object) -> Lisp_Symbol:
    assert SYMBOLP(a)
    return a
...
# }
def CHECK_SYMBOL(x: Lisp_Object):
    return CHECK_TYPE(SYMBOLP(x), Q.symbolp, x)
#
...
PSEUDOVECTOR_FLAG = PTRDIFF_MAX - PTRDIFF_MAX // 2
...
MOST_NEGATIVE_FIXNUM = (-1 - MOST_POSITIVE_FIXNUM)
#
...
# }
def XFIXNUM_RAW(a: Lisp_Object) -> int:
    return a
#
# INLINE Lisp_Object
# make_ufixnum (EMACS_INT n)
...
#   return a;
# }
def make_lisp_ptr(ptr: TLisp, type: Type[TLisp]) -> TLisp:
    assert isinstance(ptr, type)
    return ptr
#
...
def XSETSYMBOL(b): return make_lisp_ptr(b, Lisp_Symbol)
...
class Lisp_Cons:
    ...
    #     Lisp_Object car;
    car: Lisp_Object = None
    #
    #     union
    #     {
    #	/* Cdr of this cons cell.  */
    #	Lisp_Object cdr;
    cdr: Lisp_Object = None
    #
...
# }
def NILP(x):
    return x is None or EQ(x, Q.nil)
...
# }
def CONSP(x: Lisp_Object):
    return isinstance(x, Lisp_Cons)
...
# }
def CHECK_CONS(x: Lisp_Object):
    return CHECK_TYPE(CONSP(x), Q.consp, x)
...
# }
def XCONS(a: Lisp_Object) -> Lisp_Cons:
    CHECK_CONS(a)
    return a
...
#   *xcar_addr (c) = n;
# }
def XSETCAR(c: Lisp_Object, n: Lisp_Object):
    XCONS(c).car = n
...
#   *xcdr_addr (c) = n;
# }
def XSETCDR(c: Lisp_Object, n: Lisp_Object):
    XCONS(c).cdr = n
...
#   return Qnil;
# }
def CAR(c: Lisp_Object):
    if CONSP(c):
        return XCAR(c)
    elif not NILP(c):
        wrong_type_argument(Q.listp, c)
    return Q.nil
...
# }
def CAR_SAFE(c: Lisp_Object):
    return XCAR(c) if CONSP(c) else Q.nil
...
# }
def CDR_SAFE(c: Lisp_Object):
    return XCDR(c) if CONSP(c) else Q.nil
...
# }
def STRINGP(x: Lisp_Object):
    return isinstance(x, Lisp_String)
...
# }
def CHECK_STRING(x: Lisp_Object):
...
# }
def XSTRING(a: Lisp_Object) -> Lisp_String:
    assert STRINGP(a)
    return a
...
# }
def STRING_MULTIBYTE(str: Lisp_Object) -> bool:
    return 0 <= XSTRING(str).u.s.size_byte
#
...
STRING_BYTES_BOUND = min(MOST_POSITIVE_FIXNUM, min(SIZE_MAX, PTRDIFF_MAX) - 1)
#
...
#   } while (false)
def STRING_SET_UNIBYTE(STR: Lisp_Object):
    if XSTRING(STR).u.s.size == 0:
        return G.empty_unibyte_string
    else:
        XSTRING(STR).u.s.size_byte = -1
        return STR
#
...
# }
def SDATA(string: Lisp_Object) -> bytearray:
    if isinstance(string, str):
        return bytearray(string.encode('utf-8'))
    else:
        # return XSTRING(string).u.s.data.decode('utf-8')
        return XSTRING(string).u.s.data
...
# }
def SSDATA(string: Lisp_Object) -> bytearray:
    return SDATA(string)
...
# }
def SREF(string: Lisp_Object, index: int) -> int:
    return SDATA(string)[index]
...
#   return nchars;
# }
def SCHARS(string: Lisp_Object) -> int:
    nchars = XSTRING(string).u.s.size
    assert 0 <= nchars
    return nchars
#
...
#   return nbytes;
# }
def STRING_BYTES(s: Lisp_String) -> int:
    nbytes = s.u.s.size if s.u.s.size_byte < 0 else s.u.s.size_byte
    assert 0 <= nbytes
    return nbytes
#
# INLINE ptrdiff_t
...
# }
def SBYTES(string: Lisp_Object) -> int:
    return STRING_BYTES(XSTRING(string))
# INLINE void
...
@dataclasses.dataclass
class Lisp_Vector:
    contents: List[Lisp_Object] = dataclasses.field(default_factory=list)

Lisp_Vectorlike = Lisp_Vector
...
# }
def XVECTOR(a: Lisp_Object):
    assert VECTORLIKEP(a)
    return a
#
# INLINE ptrdiff_t
...
# }
def VECTORP(x):
    # return VECTORLIKEP(x) and not
    return VECTORLIKEP(x) and not (ASIZE(x) & PSEUDOVECTOR_FLAG)
#
# INLINE void
...
# }
def CHECK_VECTOR(x: Lisp_Object):
    return CHECK_TYPE(VECTORP(x), Q.vectorp, x)
#
#
...
# }
def BOOL_VECTOR_P(a: Lisp_Object):
    print("TODO: BOOL_VECTOR_P")
    return False
#
# INLINE void
...
# }
def CHAR_TABLE_P(a: Lisp_Object):
    print("TODO: CHAR_TABLE_P")
    return False
#
...
# }
def SUBRP(a: Lisp_Object):
    return isinstance(a, Lisp_Subr)
...
# }
def XSUBR(a: Lisp_Object) -> Lisp_Subr:
    PP.eassert("SUBRP (a)", globals(), locals())
    return a
...
# }
def SET_SYMBOL_VAL(sym: Lisp_Symbol, v):
    assert sym.u.s.redirect == SYMBOL_PLAINVAL
    sym.u.s.val.value = v
...
# }
def FIXNATP(x: Lisp_Object):
    return FIXNUMP(x) and 0 <= XFIXNUM(x)
#
...
# }
def COMPILEDP(a: Lisp_Object):
    print("TODO: COMPILEDP")
    return False
#
# INLINE bool
...
# }
def ARRAYP(x: Lisp_Object):
    return VECTORP(x) or STRINGP(x) or CHAR_TABLE_P(x) or BOOL_VECTOR_P(x)
# 
# INLINE void
...
# }
def CHECK_ARRAY(x: Lisp_Object, predicate: Lisp_Object):
    return CHECK_TYPE(ARRAYP(x), predicate, x)
# INLINE void
# CHECK_FIXNAT (Lisp_Object x)
# {
#   CHECK_TYPE (FIXNATP (x), Qwholenump, x);
# }
def CHECK_FIXNAT(x: Lisp_Object):
    return CHECK_TYPE(FIXNATP(x), Q.wholenump, x)
#
# INLINE double
...
# }
def CHECK_NUMBER(x):
    return CHECK_TYPE(NUMBERP(x), Q.numberp, x)
#
# INLINE void
...
# }
def CHECK_INTEGER(x):
    return CHECK_TYPE(INTEGERP(x), Q.numberp, x)
#
# INLINE void
...
# }
def CHECK_SUBR(x):
    return CHECK_TYPE(SUBRP(x), Q.subrp, x)
# 
#
...
def CALLMANY(f: Callable[..., TLisp], array: Tuple[Lisp_Object]) -> TLisp:
    return f(*array)
#
...
def CALLN(f: Callable[..., TLisp], *args) -> TLisp:
    return CALLMANY(f, args)
#
...
# }
def set_string_intervals(s: Lisp_Object, i: INTERVAL):
    XSTRING(s).u.s.intervals = i
#
...
# }
def build_pure_c_string(s: str) -> Lisp_String:
    from . import alloc
    return alloc.make_pure_c_string(s, strlen(s))
#
...
# }
def xsignal(error_symbol: Lisp_Object, data: Lisp_Object):
    return F.signal(error_symbol, data)
...
#       }								 \
#   } while (false)
def SAFE_NALLOCA(multiplier, nitems):
    return [None] * nitems
#
...
def FOR_EACH_TAIL(tail) -> Generator[Tuple[Lisp_Cons, Callable[[Lisp_Cons], Lisp_Cons]]]:
    return FOR_EACH_TAIL_INTERNAL(tail, lambda: circular_list(tail), true)
#
...
def FOR_EACH_TAIL_SAFE(tail) -> Generator[Tuple[Lisp_Cons, Callable[[Lisp_Cons], Lisp_Cons]]]:
    return FOR_EACH_TAIL_INTERNAL(tail, lambda: Q.nil, false)
#
...
# };
@dataclasses.dataclass
class for_each_tail_internal:
    tortoise: Lisp_Object
    max: int
    n: int
    q: int
#
...
# 	 && EQ (tail, li.tortoise))					\
# 	? (cycle) : (void) 0))
def FOR_EACH_TAIL_INTERNAL(tail: Lisp_Cons, cycle: Callable, check_quit: bool):
    def set_tail(newval):
        nonlocal tail
        tail = newval
        return newval
    # li = for_each_tail_internal(tail, 2, 0, 2)
    # while CONSP(tail):
    #     li.q -= 1
    #     def check():
    #         if check_quit:
    ...
    #         li.tortoise = tail
    #         return false
    #     if (li.q != 0 or check() or check2()) and EQ(tail, li.tortoise):
    #         return cycle()
    #     yield tail, set_tail
    #     tail = XCDR(tail)
    tortoise = tail
...
Lisp_Obarray = List[Lisp_Symbol]

def wrong_type_argument(predicate: Lisp_Object, value: Lisp_Object):
    from . import data
    return data.wrong_type_argument(predicate, value)

def maybe_quit():
    from . import eval
    return eval.maybe_quit()

def circular_list(list: Lisp_Object):
    from . import data
    return data.circular_list(list)
