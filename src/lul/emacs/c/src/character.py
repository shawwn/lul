...
# }
def CHAR_VALID_P(c: int):
    return 0 <= c and c <= MAX_CHAR
#
...
# }
def CHARACTERP(x: Lisp_Object):
    return FIXNUMP(x) and CHAR_VALID_P(XFIXNUM(x))
#
...
# }
def CHECK_CHARACTER(x: Lisp_Object):
...
#           + (MAX_4_BYTE_CHAR < c));
# }
def CHAR_BYTES(c) -> int:
    return ((-2 if MAX_5_BYTE_CHAR < c else 1)
            + (MAX_1_BYTE_CHAR < c)
            + (MAX_2_BYTE_CHAR < c)
            + (MAX_3_BYTE_CHAR < c)
            + (MAX_4_BYTE_CHAR < c))
